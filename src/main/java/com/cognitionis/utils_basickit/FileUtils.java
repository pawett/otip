...
public class FileUtils {
	...
	public static String getExecutionPath() {
		try {
			...
			Logger.WriteError("Errors found (FileUtils):\n\tApplication execution path not found: "
					+ e.getMessage() + "\n", e);
			return "";
		}
	}
	...
	public static boolean URL_exists(String URLName) {
		...
		try {
			...
			Logger.WriteDebug("EXISTS? " + ensureURL(URLName));
			...
			Logger.WriteError("Errors found (FileUtils): URL (" + URLName
	...
	public static String getResourcesPath(String subdir) throws Exception {
		...
		if (!URL_exists(res_path)) { // Check for external resoucre (outside
			...
			Logger.WriteDebug("look outside classes: "
						+ ensureURL(res_path));
		
		}

		if (!URL_exists(res_path)) { // Check for JAR resoucre
			Logger.WriteDebug("look into jar");
			...
			if (res == null) {
				Logger.WriteDebug("java jar res not found " + subdir);
				
			} else {
				Logger.WriteDebug("java jar res " + res.toString());
				Logger.WriteDebug("file: " + new File(res.getPath())); // path
					// part
					// of
					// the
				// URL
				...
				final Enumeration<URL> resources = FileUtils.class
						.getClassLoader().getResources(
								subdir.replaceAll("\\\\", "/")); //
				Logger.WriteDebug("exists = " + resources.hasMoreElements());
