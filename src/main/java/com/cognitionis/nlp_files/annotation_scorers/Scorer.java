...
public class Scorer {
    ...
    public Score score(TokenizedFile annot, String key, int elembiocol, int attrscol) {
        ...
        try {
            ...
            while ((keyline = keyreader.readLine()) != null) {
                ...
                annotline=annotline.trim();keyline=keyline.trim();
                // check end of sentence and save judgements if needed
                if (keyline.length()==0 || keyline.equals("|")) {
                    if (annotline.length()==0 || annotline.equals("|")) {
                ...
                String[] keyarr = keyline.split(annot.getFieldSeparatorRE());
                String[] annotarr = annotline.split(annot.getFieldSeparatorRE());

                if (keyarr.length < annot.getNumFields() || annotarr.length < annot.getNumFields()) {
    ...
    public Score score_recclass(TokenizedFile annot, String key, int elembiocol, int attrscol) {
        // recorrer ambdos fitxers de forma síncrona i anar calculat els scores segons el constructor
        Score score = null;
        int numline = 0;
        try {
    ...
    // LA CLAVE pot retornar un objecte score del qual jo puc adquirir els differents scores/measures implementades..
    public Score autoscore(TokenizedFile annot, int keybiocol) {
        // recorrer ambdos fitxers de forma síncrona i anar calculat els scores segons el constructor
        Score score = null;
        int numline = 0;
        ...
        try {
            ...
            while ((annotline = annotreader.readLine()) != null) {
                ...
                if (annotarr.length < annot.getNumFields()) {
